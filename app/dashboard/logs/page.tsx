...
export default function LogsPage() {
  ...
  const filteredLogs = logs.filter((log) =>
    !filter || log.service.toLowerCase().includes(filter.toLowerCase()) ||
        ...
        <div className="max-w-3xl mx-auto space-y-4">
          ...
          {!loading && logs.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {Object.entries(logs.reduce((acc, l) => { acc[l.risk_tier] = (acc[l.risk_tier] || 0) + 1; return acc; }, {} as Record<string, number>))
                .map(([tier, count]) => (
                  <button key={tier} onClick={() => setFilter(filter === tier ? "" : tier)}
                    className={`text-[10px] font-mono font-medium px-2 py-1 rounded border transition-colors ${
                      filter === tier ? tierColors[tier] || "text-zinc-400" : "text-zinc-600 bg-zinc-900/50 border-zinc-800 hover:border-zinc-700"
                    }`}>
                    {tier} ({count})
                  </button>
                ))}
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              <p className="text-sm text-zinc-600">{logs.length === 0 ? "No actions logged yet." : "No logs match your filter."}</p>
            </div>
          ) : (
            <div className="space-y-2">
              {filteredLogs.map((log) => {
                ...
                return (
                  <div key={log.id} className="rounded-xl border border-zinc-800/50 bg-zinc-900/30 overflow-hidden">
                    <button onClick={() => setExpandedId(isExpanded ? null : log.id)}
                      ...
                      <div className="hidden sm:block w-20 shrink-0">
                        <p className="text-[10px] text-zinc-600">{formatDate(log.created_at)}</p>
                        <p className="text-[10px] font-mono text-zinc-500">{formatTime(log.created_at)}</p>
                      </div>
                      <span className="text-[10px] font-medium text-zinc-400 bg-zinc-800/50 border border-zinc-800 rounded px-1.5 py-0.5 shrink-0">{log.service}</span>
                      <span className={`text-[10px] font-mono font-medium px-1.5 py-0.5 rounded border shrink-0 ${tierColors[log.risk_tier] || "text-zinc-400"}`}>{log.risk_tier}</span>
                      <span className="text-xs text-zinc-400 truncate flex-1 min-w-0">{log.action_type}</span>
                      <span className={`text-[10px] font-medium shrink-0 ${statusColors[log.status] || "text-zinc-500"}`}>{log.status}</span>
                      ...
                      <div className="px-4 pb-4 border-t border-zinc-800/50 space-y-3 pt-3">
                        {log.description && (
                          <div>
                            ...
                            <p className="text-xs text-zinc-400 break-words">{log.description}</p>
                          </div>
                        )}
                        {log.reasoning && (
                          <div>
                            <p className="text-[10px] text-zinc-600 uppercase tracking-wider mb-1">Reasoning</p>
                            <p className="text-xs text-zinc-400">{log.reasoning}</p>
                          </div>
                        )}
                        {log.scopes_used && log.scopes_used.length > 0 && (
                          <div>
                            <p className="text-[10px] text-zinc-600 uppercase tracking-wider mb-1">Scopes</p>
                            <div className="flex flex-wrap gap-1.5">
                              {log.scopes_used.map((scope) => (
                                <span key={scope} className="text-[10px] font-mono text-zinc-500 bg-zinc-800/50 border border-zinc-800 rounded px-1.5 py-0.5">{scope}</span>
                              ))}
                            </div>
                          </div>
                        )}
